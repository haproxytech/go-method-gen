...
type Options struct {
	...
	FieldNamesToSkip string // Comma separated list of field names to skip in function generation.
	ProcessInterface bool   // flag to process or skip interface in type.
}
...
func Generate(types []reflect.Type, opts Options) error {
	...
	if opts.FieldNamesToSkip != "" {
		for _, fieldName := range strings.Split(opts.FieldNamesToSkip, ",") {
			fieldNamesToSkip[strings.TrimSpace(fieldName)] = struct{}{}
		}
	}
	...
	for _, typ := range types {
		...
		parser.Parse(root, typ, typ.PkgPath(), map[string]struct{}{}, parser.ParserOptions{
			FieldNamesToSkip: fieldNamesToSkip,
			ProcessInterface: opts.ProcessInterface,
		})
	}
